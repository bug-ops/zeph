...
| `agent::tool_execution` | Tool call handling, redaction, result processing; emits `LoopbackEvent::ToolStart` (with UUID) before each tool executes and `LoopbackEvent::ToolOutput` (with matching UUID and `is_error`) after; `call_llm_with_retry()` / `call_chat_with_tools_retry()` — auto-detect `ContextLengthExceeded`, compact context, and retry (max 2 attempts); `prune_stale_tool_outputs` invokes `count_tokens` once per `ToolResult` part |
