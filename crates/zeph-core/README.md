...
| `agent::persistence` | Message persistence and background graph extraction integration; `maybe_spawn_graph_extraction` fires a `SemanticMemory::spawn_graph_extraction` task per user turn with injection-flag guard and last-4-user-messages context window (feature-gated: `graph-memory`) |
...
| `experiments` | Benchmark dataset & LLM-as-Judge evaluation engine (feature-gated: `experiments`): `BenchmarkSet` / `BenchmarkCase` loaded from TOML files via `from_file()` with validation; `Evaluator` runs each case against a subject model then scores responses in parallel using a judge model via `chat_typed_erased` structured output (`JudgeOutput` schema — score + reason); `CaseScore` per-case results with latency and token tracking; `EvalReport` aggregate metrics (mean score, p50/p95 latency percentiles, total tokens, partial-run detection); semaphore-based concurrency (`DEFAULT_PARALLEL_EVALS = 3`), per-invocation token budget enforcement via `AtomicU64` counter, XML boundary tags (`<subject_response>`) with entity escaping for subject response isolation; `EvalError` typed errors for load/parse/budget/judge failures |
