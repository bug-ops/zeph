...
Requires Rust 1.98+. Native async traits are used throughout. `async-trait` is retained only in crates blocked by upstream dependencies (`zeph-core`, `zeph-mcp`, `zeph-acp` — blocked by `rmcp`).
