...
| **Self-experimentation** | Autonomous LLM config experimentation engine (inspired by [autoresearch](https://github.com/karpathy/autoresearch)). Iteratively varies temperature, top-p, system prompt phrasing, and other parameters, evaluates each variant via LLM-as-judge scoring, and keeps improvements that pass a configurable threshold. Optional scheduled runs via cron. Enabled by the `experiments` feature flag (opt-in, included in `full`). |
