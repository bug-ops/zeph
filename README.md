...
| **Semantic memory** | SQLite + Qdrant with MMR re-ranking, temporal decay, query-aware memory routing (keyword/semantic/hybrid), cross-session recall, implicit correction detection, and credential scrubbing. Optional **graph memory** adds entity-relationship tracking with BFS traversal for multi-hop reasoning and temporal fact tracking. Background LLM extraction runs fire-and-forget on each turn; graph facts are injected into the context window alongside semantic recall. [→ Memory](https://bug-ops.github.io/zeph/concepts/memory.html) · [→ Graph Memory](https://bug-ops.github.io/zeph/concepts/graph-memory.html) |
...
| **Task orchestration** | DAG-based task graphs with dependency tracking, parallel execution, configurable failure strategies (abort/retry/skip/ask), timeout enforcement, and SQLite persistence. LLM-powered goal decomposition via `Planner` trait — structured output is validated and converted into a `TaskGraph` with DAG cycle detection. Decompose complex goals into sub-agent tasks with automatic scheduling and result aggregation. Feature-gated: `orchestration`. [→ Orchestration](https://bug-ops.github.io/zeph/advanced/orchestration.html) |
