...
- `handle_native_tool_calls()` now pushes `ToolResult` parts for all tool calls in the batch before propagating any error from `attempt_self_reflection`. Previously, when the self-reflection LLM call itself returned `Err`, the function propagated the error without emitting any `ToolResult` messages, leaving every `ToolUse` in the assistant message orphaned and causing Claude API 400 errors on the next request (#1517)
