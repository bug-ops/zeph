...
- `skill_metrics()` and `load_skill_outcome_stats()` now exclude `user_approval`/`user_rejection` outcomes from execution-based success rate calculations, preventing explicit user feedback from polluting automated skill quality metrics
