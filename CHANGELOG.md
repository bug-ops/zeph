...
- feat(classifiers): `CandleNerClassifier` — Candle-backed DeBERTa-v2 NER model with BIO/BIOES span decoding; `NerSpan { label, score, start, end }` struct; `ClassificationResult` extended with `spans: Vec<NerSpan>` (empty for sequence classifiers); piiranha (`iiiorg/piiranha-v1-detect-personal-information`) default NER model; `[classifiers] ner_model` config field; chunked inference with span deduplication for overlap regions (#2211)
- feat(learning): `DetectorMode::Model` variant — ML-backed correction detection via `ClassifierBackend`; new `detector_model` field in `[skills.learning]`; `detect_with_model()` async method on `FeedbackDetector`; `with_feedback_classifier()` builder method on `Agent`; bootstrap wires `CandleNerClassifier` when `detector_mode = "model"` and `classifiers.enabled = true`; falls back to regex with `warn!()` when backend is absent or feature is disabled; `--init` wizard updated with `model` option; bootstrap validates non-empty `detector_model` and warns on empty (#2210)
