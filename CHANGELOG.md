...
- `RUST_LOG` ignored in CLI mode when compiled with `--features full`: `tracing_subscriber::fmt::init()` used default ERROR filter instead of `EnvFilter`. Replace all 7 bare `fmt::init()` calls with `init_subscriber()` which respects `RUST_LOG` env var. Affects main agent loop, subcommands (skill, memory, ingest, sessions, agents), and daemon mode (#1368)
