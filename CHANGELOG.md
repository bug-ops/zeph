...
- Add `//!` module doc comment to `zeph-core::text` for rustdoc consistency with `net` and `math` modules (REV-01)
- Remove redundant local `truncate_chars` from `orchestration/aggregator.rs`; use canonical `crate::text::truncate_chars` (REV-02)
- Change `pub(crate) mod helpers` to `pub(super) mod helpers` in `zeph-acp/src/agent/mod.rs` to align module visibility with `pub(super)` function visibility (REV-04)
- Replace duplicate cosine_similarity edge-case tests in `sqlite_vector_store.rs` and `in_memory_store.rs` with single smoke tests; full coverage remains in `math.rs` (REV-03)
