...
| FR-B-011 | WHEN any node dispatches (spawn or `RunInline`) under a graph with `[orchestration.command].enabled = true` THE SYSTEM SHALL have zeph-core append a `<shared-state>` block (from `store_list("orch/{graph_id}", owner_key, …)`, capped at a fixed row count — GitHub #6763 — with the surviving rows ordered by `(namespace, key)`), wrapped as untrusted/spotlighted content, to the prompt `router.rs::build_task_prompt` produced — `router.rs` itself remains store-free. WHEN the namespace holds more rows than the cap THE SYSTEM SHALL mark the block's opening tag `truncated="true" shown="N"` (trusted framing, outside the sanitized body) so the receiving node can distinguish an absent key from one dropped by truncation | must |
