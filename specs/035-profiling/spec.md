...
- `TracingGuards` (`log_guard` rolling-file `WorkerGuard`, and the profiling-gated Chrome trace flush guard) MUST flush on every process-exit path reachable after `init_tracing`, not just the normal `Drop` on function return: `SIGTERM` (via a take-once `LogGuardCell`-backed listener installed only when nothing else on the invocation path already owns `SIGTERM` — `--daemon`, `zeph serve-sessions`, `zeph scheduler serve` each install their own graceful-shutdown handler), `Ctrl-C`/`SIGINT` (`tokio::signal::ctrl_c()`, cross-platform), and every `std::process::exit`/early-return call site in `src/runner.rs` and `src/commands/*.rs` reachable after tracing init (`exit_with_flush` helper, or plain `anyhow`/`?` propagation so an `Err` return unwinds through normal `Drop`). `std::process::exit` runs no destructors and bypasses `Drop`-based flush entirely — every new CLI exit path added after tracing init must route through one of these two mechanisms, not a bare `std::process::exit` (#6683, #6693, #6696, #6697, #6698, #6708, #6709).
- The SIGTERM flush listener re-raises the signal via `signal_hook::low_level::emulate_default_handler` after flushing, so the process still dies *by the signal* (`WIFSIGNALED`) as external supervisors (systemd, launchd, container runtimes, `zeph scheduler stop`'s wait loop) expect — never substitute a plain `exit(143)`.
- Local Chrome trace layer MUST use `tracing_chrome::TraceStyle::Async`, not `Threaded` — `Threaded` fires `on_enter`/`on_exit` on every poll of an async span, fragmenting any span containing an `.await` into many short on-CPU slices instead of one continuous wall-clock duration (#6682).
