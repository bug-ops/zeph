...
- The `Retrieve` action's "retrieve context, then retry" mandate MUST NOT loop forever. Two independent bounds apply per turn, neither alone sufficient for every reachable path: (1) once `memory_search` — the retrieval tool the mandate's hint recommends — fails with a retryable/network-class error or `ConfirmationRequired`, the gate stops mandating further detours and lets the originally-requested tool proceed directly instead (`ToolOrchestrator::has_blocked_retrieval_this_turn`); this reliably breaks the loop when the user confirms and the re-execution itself still fails, but self-heals back to allowing a new mandate if the user instead declines (a successful `[cancelled by user]` outcome clears the recorded failure); (2) regardless of cause, the gate mandates at most `MAX_RETRIEVE_MANDATES_PER_TURN` (3) *consecutive* `Retrieve` cycles before falling back to direct execution — this is the bound that actually terminates the loop for sub-cases (1) cannot reach (e.g. repeated declines) or any other undiscovered variant, such as the LLM retrying a different but similarly-gated tool each cycle rather than the exact original call (#5774). The consecutive-mandate counter (`ToolOrchestrator::retrieve_mandate_count`) resets via `reset_retrieve_mandate_count` **only when a tool other than `memory_search` reaches real dispatch**. `memory_search` itself always reaches real dispatch, because its gain score (0.8) is high enough to take the direct `ToolCall` branch regardless of turn uncertainty — so a reset that did not exclude it would clear the counter on every cycle of the repeated-decline loop, and the breaker would never trip for that sub-case, defeating bound (2) precisely where bound (1) cannot help.
